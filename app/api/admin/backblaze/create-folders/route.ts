...
export async function POST(request: NextRequest) {
  try {
    ...
    console.log('[v0] Getting bucket ID...')
    ...
    console.log('[v0] Bucket ID:', bucketId)
    ...
    for (const [parent, subcategories] of Object.entries(CATEGORIES)) {
      for (const subcategory of subcategories) {
        const folderPath = `VIDS/Categories/${parent}/${subcategory}`
        ...
        try {
          ...
          if (uploadResponse.ok) {
            ...
            console.error('[v0] Failed to create folder:', folderPath, uploadResponse.statusText)
            results.push({ path: folderPath, status: 'failed', error: uploadResponse.statusText })
            failed++
          }
        } catch (error) {
          console.error('[v0] Error creating folder:', folderPath, error)
          results.push({
            path: folderPath,
            status: 'failed',
    ...
    console.log('[v0] Folder creation summary:', summary)

    ...
    for (const [parent, subcategories] of Object.entries(CATEGORIES)) {
      for (const subcategory of subcategories) {
        ...
        try {
          ...
          const markerFileName = `${folderPath}/.folder_marker`
          const markerResponse = await fetch(uploadUrl, {
            method: 'POST',
            headers: {
              Authorization: uploadToken,
              'X-Bz-File-Name': markerFileName,
          ...
          if (markerResponse.ok) {
            created++
            results.push({ path: folderPath, status: 'created' })
          } else {
            failed++
            results.push({ path: folderPath, status: 'failed', error: markerResponse.statusText })
          }
        } catch (error) {
          failed++
          results.push({

export class BackblazeNativeStorage {
  ...
  private async authenticate(): Promise<void> {
    console.log("[v0] Authenticating with Backblaze B2 native API...")
  ...
  private async getBucketId(): Promise<string> {
    ...
    try {
      const response = await fetch(`${this.apiUrl}/b2api/v3/b2_list_buckets`, {
        ...
        body: JSON.stringify({
          accountId: this.keyId.substring(0, 12), // Extract account ID from key ID
        }),
      })
  ...
  async uploadFile(fileName: string, file: File, contentType: string): Promise<string> {
    console.log("[v0] Starting native B2 upload for:", fileName)
    ...
    try {
      ...
      if (!uploadUrlResponse.ok) {
        ...
        console.log("[v0] Get upload URL error:", uploadUrlResponse.status, errorText)
      ...
      console.log("[v0] Native B2 upload successful:", fileUrl)
      return fileUrl
    } catch (error) {
      console.log("[v0] Native B2 upload error:", error)
      throw error
    }
  }
